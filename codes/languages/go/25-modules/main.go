...
^ will also tell the other user that this repo is going to be utilised so better get it
- Adds an entry of this module along with exact version in `go.mod` file
- For added layer of security via checksum it adds hash to that particular version in `go.sum` file

	>>> go mod verify >>> Can be used to verify the checksum of packages used

btw the external packages actually are placed in "$GOHOME/pkg/mod/cache/download"

3. `go mod tidy`
NOTE will fix the `go.mod` and `go.sum` files
If something was there but not used -> will be removed after it
If something was there and also being used but says `//indirect` -> will remove the indirect
...
github.com/gorilla/mux v1.2.0 v1.3.0 v1.4.0 v1.5.0 v1.6.0 v1.6.1 v1.6.2 v1.7.0 v1.7.1 v1.7.2 v1.7.3 v1.7.4 v1.8.0 -> Existing versions of the package to know directly from the terminal and utilise if required (version history)
...
### Every external package is in cache dir, so if you deploy to some server how you gonna do that?
### OR say you wanna modify some bit of the code in the package itself and then use it..
  ...
  - and now force go to use the things inside the project's dir
...
func main() {
	...
	// 		      ^ 	  ^                 ^
	// 		  	  |   	  |                 |
	//  Responding to   What function   What kind of
	//   requests in     to call..  request so that we can respond
	//     homedir

	// Running a server in golang - is literally a one liner command!
	log.Fatal(http.ListenAndServe(":4000", r)) // log.Fatal logs whenver something goes wrong
	// 		  		        		^      ^
	// 		  	                    |      |
	//                     		  Host 	 router
	//                 		Hostname:Port
	//                By default Hostname=127.0.0.1
}
...
func homeServer(w http.ResponseWriter, r *http.Request) {
	...
	w.Write([]byte(`<h1>Hello from proffapt</h1>`)) // putting string inside []byte within `` will convert it into bytes
	//         ^ We received everything from the web in []byte format so that means it's the standard so that's exactly what we gonna send
}
